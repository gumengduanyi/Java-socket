...
## 文件：`server/parser/Parser.java`（补充）

```java
result = "A01-" + sd.getGrade(para);
```

说明：这行代码是 `Parser.commandProcess` 中处理 `A01` 命令的核心，它把从 `StudentDAO` 获得的成绩拼接成协议响应并返回给客户端。

- 行为细分：
    - `sd.getGrade(para)`：调用 `StudentDAO.getGrade`，参数 `para` 是从客户端请求中解析出的学生学号（例如 `"2019001"`）。
    - `sd.getGrade(para)` 的返回值：由 `FileOperation` 实现决定，通常为查到的成绩（如 `"95"`），若未找到则返回 `"None"`。
    - 将返回值与前缀 `"A01-"` 拼接，得到最终响应字符串（例如 `"A01-95"` 或 `"A01-None"`）。

- 举例（完整链路的部分）：
    1. 客户端发送请求：`"A01-2019001"`。
    2. `Parser.run` 拆分为 `command = "A01"`，`para = "2019001"`，调用 `commandProcess("A01","2019001")`。
    3. 在 `commandProcess` 中执行 `StudentDAO sd = new StudentDAO(); sd.getGrade("2019001")`。
    4. `FileOperation` 在 `student.txt` 查找到 `2019001,张三,CS,95`，返回 `"95"`。
    5. 这行代码把返回值拼接成 `"A01-95"`，并最终由服务器回写给客户端。

- 边界与注意点：
    - 若 `sd.getGrade(para)` 返回 `"None"`，客户端会收到 `"A01-None"`，`CmdView` 的解析逻辑会把它识别为无成绩或未找到。
    - `Parser` 当前只处理 `A01`（和占位的 `A02`），若协议扩展需要增加新的命令，应在 `commandProcess` 中增加对应分支。
    - `StudentDAO` 构造 `new FileOperation("student.txt")`：若文件路径不正确或文件缺失，会在 `FileOperation` 中抛出/打印异常并导致返回 `"None"`，建议在部署时确认 `student.txt` 的相对路径与运行目录一致。
    - 数据校验：`Parser` 使用简单的 `split("-")` 分割命令与参数，未对参数格式做严格校验（比如空字符串、非法字符），建议在生产代码中加入参数验证与错误返回码。

- 建议改进：
    - 将 `StudentDAO` 改为通过构造器注入 `DataAble` 实例（依赖注入），便于测试与替换实现。例如：
        ```java
        public class Parser {
                private DataAble dao;
                public Parser(DataAble dao){ this.dao = dao; }
                // 使用 dao.getGrade(para)
        }
        ```
    - 将响应格式抽象为常量或枚举，避免硬编码字符串前缀 `"A01-"`，并在协议规范中注明错误码和格式。

（以上为对 `Parser` 中该行代码的补充说明，已放在文档中以便查阅）
